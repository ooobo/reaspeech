...
impl TDTModelConfig {
    /// Create config by reading vocab size from vocab.txt
    pub fn from_vocab_file<P: AsRef<Path>>(vocab_path: P) -> Result<Self> {
        let file = File::open(vocab_path.as_ref())
            .map_err(|e| Error::Config(format!("Failed to open vocab.txt: {}", e)))?;
        let reader = BufReader::new(file);
        let vocab_size = reader.lines().count();

        if vocab_size == 0 {
            return Err(Error::Config("vocab.txt is empty".to_string()));
        }
...
impl Default for TDTModelConfig {
    fn default() -> Self {
        Self {
            vocab_size: 8193, // Fallback for v3
        }
    }
}
...
impl ParakeetTDTModel {
    ...
    pub fn from_pretrained<P: AsRef<Path>>(
        ...
        // Try to read vocab size from vocab.txt, fall back to default (v3) if not found
        let vocab_path = model_dir.join("vocab.txt");
        let config = if vocab_path.exists() {
            TDTModelConfig::from_vocab_file(&vocab_path)?
        } else {
            TDTModelConfig::default()
        };
